...
import {calculatePointOnCircle, getPieChartPiecePath} from "../utils/pie-chart";
...
const CommunityDashboardTab = () => {
    ...
    const {data: membersPowerShare} = useCommunityMembersPowerShare(selectedCommunity?.id || '', {enabled: !!selectedCommunity})
        ...
        return membersPowerShare.map((member, index, array) => {
            if(index === 0)
                return {
                    ...
                    from: 0,
                    to: member.share * 360,
                    textPosition: calculatePointOnCircle(70, member.share * 360 / 2, {x: 100, y: 100})
                }
            else
                return {
                    user: member.user,
                    ...
                    textPosition: calculatePointOnCircle(70, array[index - 1].share * 360 + member.share * 360 / 2, {x: 100, y: 100})
                }
        })
    }, [membersPowerShare])
    ...
    return (
        <ScrollView className='my-5 mx-4 flex'>
            ...
            <Svg viewBox='0 0 200 200' className='w-full' style={{ height: 300, backgroundColor: '#1C1B2D'}}>
                {pieChartData?.map((data, index) => (
                    <><Path key={index} d={getPieChartPiecePath(100, data.from, data.to)} fill={colors[index]} stroke={'#1C1B2D'} strokeWidth={2}/>
                    <SvgText stroke='white' textAnchor='middle' transform={`rotate(0, ${data.textPosition.x}, ${data.textPosition.y})`} x={data.textPosition.x} y={data.textPosition.y} fontSize={10} fontWeight={1}>{data.user}</SvgText></>
                ))}
            </Svg>
        </ScrollView>
