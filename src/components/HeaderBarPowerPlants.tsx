...
const HeaderBarPowerPlants = () => {
    const {selectedPowerPlant, setSelectedPowerPlant} = usePowerPlantStore(state => ({selectedPowerPlant: state.selectedPowerPlant, setSelectedPowerPlant: state.setSelectedPowerPlant}));
    const {toggleOpened} = useSideMenuStore()
    const [dropdownOpened, setDropdownOpened] = useState(false);
    const {data: powerPlants} = usePowerPlants()
    const dropdownItems = useMemo(() => powerPlants?.map(powerPlant => ({label: powerPlant.displayName, id: powerPlant._id})) || [], [powerPlants])
    ...
    return (
        <View className='flex justify-between items-center flex-row dark:bg-dark-main p-4'>
            <TouchableOpacity activeOpacity={0.6} onPress={toggleDropdown} className='flex flex-row items-center'>
                <Text className='text-2xl text-white font-bold'>{selectedPowerPlant.name}</Text>
