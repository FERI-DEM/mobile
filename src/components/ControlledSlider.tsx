    ...
    <View className={twMerge('mt-7', classNameContainer)}>
      {label && (
        <View className="flex flex-row items-center justify-between">
          <Text className={twMerge('dark:text-white ml-0.5', classNameLabel)}>
            {label}
          </Text>
          {showValueDisplay && (
            <Text className="dark:text-white text-right">{`${field.value} ${displayValuePostfix}`}</Text>
          )}
        </View>
      )}
      <Slider
...
export const ControlledSlider: FC<SliderProps> = ({ name, ...props }) => {
  ...
  if (!formContext || !name) {
    const msg = !formContext
      ? 'Slider must be wrapped by the FormProvider'
