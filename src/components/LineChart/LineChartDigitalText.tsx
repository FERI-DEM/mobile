...
import {createPathForRoundedCorners, prepareActiveData, prepareData} from "./helpers";
import {chars} from "./text";
import {charWidth, fontSize, innerOffset, padding, viewBoxSize, xUnit} from "./config";
...
interface LineChartDigitalTextProps {
    data: PredictedValue[]
}

const LineChartDigitalText = ({data: dataFromProps}: LineChartDigitalTextProps) => {
    const window = useWindowDimensions()
    const graphWidth = window.width - 20
    ...
    const generateCharPath = (char: string, offset: Offset) => {
    ...
    const generateTextPath = (text: string, offset: Offset) => {
...
export default LineChartDigitalText;